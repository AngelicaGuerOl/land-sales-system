...
export function LotTable({ lots, loading = false, onSelect, onEdit, onStatus }: LotTableProps) {
  ...
  return <Box className="ag-theme-balham pos-data-grid" sx={{ height: 430, width: '100%', '& .ag-header-cell-text': { fontSize: 15, fontWeight: 700 }, '& .ag-cell': { fontSize: 15 }, '& .ag-row-hover': { backgroundColor: 'action.hover !important' } }}><AgGridReact<Lot> animateRows columnDefs={columnDefs} defaultColDef={{ filter: false, resizable: true, sortable: true }} getRowId={({ data }) => data.id.toString()} theme="legacy" loading={loading} noRowsOverlayComponent={() => <Typography color="text.secondary" variant="body2">No hay lotes para mostrar.</Typography>} rowData={lots} /></Box>
}
