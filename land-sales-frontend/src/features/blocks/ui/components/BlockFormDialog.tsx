...
type Props = { open: boolean; block: LandBlock | null; pending: boolean; onClose(): void; onSubmit(input: BlockFormInput): void }
type Values = { code: string; areaM2: string; plannedLotCount: string }

export function BlockFormDialog({ open, block, pending, onClose, onSubmit }: Props) {
  const [values, setValues] = useState<Values>(() => ({ code: block?.code ?? '', areaM2: block?.areaM2 === null || block?.areaM2 === undefined ? '' : String(block.areaM2), plannedLotCount: String(block?.plannedLotCount ?? 0) }))
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (!valid) return; onSubmit({ code: values.code.trim(), areaM2: values.areaM2.trim() === '' ? null : Number(values.areaM2), plannedLotCount: Number(values.plannedLotCount), notes: block?.notes ?? null }) }
