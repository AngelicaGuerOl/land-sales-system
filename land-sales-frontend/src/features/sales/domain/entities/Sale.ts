...
export type SaleLotDetail = SaleLotInput & { code: string; blockCode: string; lotNumber: string; areaM2: number | null; financedAmount: number; outstandingBalance: number; installmentAmount: number; firstPaymentMonth: string | null; status: 'ACTIVE' | 'PAID' | 'CANCELLED'; installments: SaleInstallment[] }
